...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for org in orgs {
        ...
        handles_all_organizations.push(org.create_organization(config.get_cloned_governor()));
        handles_delete_organization.push(org.delete_organization(config.get_cloned_governor()));

        for robot in &org.robots {
            handles_all_robots.push(org.create_robot(robot, config.get_cloned_governor()));
        }
        for team in &org.teams {
            handles_all_teams.push(org.create_team(team, config.get_cloned_governor()));

            for member in &team.members.users {
                handles_all_team_members.push(org.add_user_to_team(
                    &team.name,
                    &member,
                    config.get_cloned_governor(),
                ))
            }
        ...
        for repository in &org.repositories {
            handles_all_repositories
                .push(org.create_repository(repository, config.get_cloned_governor()));
            handles_all_extra_user_permissions.push(
                org.get_user_permission_from_repository(&repository, config.get_cloned_governor()),
            );
            handles_all_extra_team_permissions.push(
                org.get_team_permission_from_repository(&repository, config.get_cloned_governor()),
            );

            handles_all_mirror_configurations
                .push(org.create_repository_mirror(&repository, config.get_cloned_governor()));
